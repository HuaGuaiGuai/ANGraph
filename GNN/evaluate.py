...
class Evaluator:
    def __init__(self, name):
        ...
        meta_info = pd.read_csv('./datasets.csv', index_col = 0)
        ...
        self.num_tasks = 1
        self.eval_metric = meta_info[self.name]['evaluation_metric']


    def _parse_and_check_input(self, input_dict):
        if (self.eval_metric == 'rocauc' or self.eval_metric == 'ap' or self.eval_metric == 'rmse'
                or self.eval_metric == 'acc' or self.eval_metric == 'r2'):
    ...
    def expected_output_format(self):
        ...
        if self.eval_metric == 'rocauc':
            ...
            desc += '{\'R2\': R2}\n'
            desc += '- R2 (float): R-Square score averaged across {} task(s)\n'.format(self.num_tasks)
        else:
            raise ValueError('Undefined eval metric %s ' % (self.eval_metric))
    ...
    def _eval_r2(self, y_true, y_pred):
        ...
        for i in range(y_true.shape[1]):
            # ignore nan values
            is_labeled = y_true[:,i] == y_true[:,i]
            y_true_ = y_true[is_labeled,i]
            y_pred_ = y_pred[is_labeled,i]
            # print("\n")
            # print(f"iter:{i} -> y_true_:{y_true_}, y_pred_:{y_pred_}, y_true_.mean():{y_true_.mean()}, "
            #       f"((y_true_ - y_pred_)**2).sum():{((y_true_ - y_pred_)**2).sum()}, "
            #       f"((y_true_ - y_true_.mean())**2).sum():{((y_true_ - y_pred_.mean())**2).sum()}")
            r2_list.append(1 - ((y_true_ - y_pred_)**2).sum() / ((y_true_ - y_true_.mean())**2).sum())
        # print("\n")
        # print(f"r2_list:{r2_list}")
        # print("\n")
        return {'r2': sum(r2_list)/len(r2_list)}
